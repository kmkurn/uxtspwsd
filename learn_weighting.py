...
ex = Experiment("xduft-learn-weighting-testrun", ingredients=[corpus_ing])
...
def learn(
    ...
    trn_samples = list(read_samples(which="train", max_length=max_length.get("train")))
    ...
    def train_on_batch(state):
        ...
        heads = torch.tensor(batch["heads"]).long().to(device)
        types = torch.tensor(batch["types"]).long().to(device)
        mask = torch.tensor(batch["words"]).long().to(device) != vocab["words"].index(
            vocab.PAD_TOKEN
        )
        assert heads.dim() == 2 and heads.shape == types.shape == mask.shape
        bsz, slen = heads.shape

        lmss = torch.tensor(batch["log_marginals_ls"]).float().to(device)
        assert lmss.shape == (bsz, len(srcs), slen, slen, len(vocab["types"]))
        lmss = rearrange(lmss, "bsz nsrc hlen dlen ntypes -> bsz nsrc dlen (hlen ntypes)")
        lmss = (src_logws.softmax(dim=0).reshape(1, -1, 1, 1) * lmss).sum(dim=1)
        assert lmss.shape == (bsz, slen, slen * len(vocab["types"]))
        ha_mask = mask.unsqueeze(2).expand(bsz, slen, len(vocab["types"])).reshape(bsz, -1)
        lmss.masked_fill_(~ha_mask.unsqueeze(1), -1e9)
        lmss = lmss.log_softmax(dim=2)  # renormalise

        idx = heads * len(vocab["types"]) + types
        llhs = lmss.gather(2, idx.unsqueeze(2)).squeeze(2)
        assert llhs.shape == (bsz, slen)
